...
import { useRecentlyAddedPorts } from "../api/queries";
...
export function RecentlyAdded() {
  ...
  const { data, isLoading, error, refetch } = useRecentlyAddedPorts({
    page: String(page),
      ...
      <div className="max-w-6xl mx-auto px-6 py-8">
        <div className="text-center py-20">
          ...
          <p className="text-sm text-[var(--color-text-secondary)] mb-4">
            Something went wrong while fetching recently added ports.
          </p>
          <button
            onClick={() => refetch()}
  ...
  return (
    <div className="max-w-6xl mx-auto px-6 py-8">
      <h1 className="text-2xl font-semibold mb-6">Recently Added Ports</h1>

      <PortList
        ports={data?.items ?? []}
        loading={isLoading}
        dateLabel="Added"
        dateField="createdInRegistryAt"
